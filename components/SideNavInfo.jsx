...
function PlaceInfo({item}){
  ...
  const isActive = (place) => {
    return place["Place Name"] === activePlace["Place Name"] ? 'bg-activePlace text-[#92D72E]' : ''
  }
  ...
  return <p onClick={() => setActivePlace(item)} className={`px-4 py-[.3rem] ${isActive(item)} font-semibold`}>{item['Place Name']}</p>
}
  ...
  return (
    <div className='min-h-screen bg-primaryDark w-[230px] flex flex-col'>
      ...
      <div className='flex flex-col border-gray-800 border-y mt-7 divide-y divide-gray-800 mx-4'>
        {
          data?.map(items => (
            <div key={items['Place Name']} className='py-2 text-white text-xs cursor-pointer'>
